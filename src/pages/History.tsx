...
//     setLoading(true);
//     try {
//       const [r, s] = await Promise.all([listRuns({ simulation_name: filterSim || undefined, status: filterStatus || undefined, limit: 100 }), listScenarios()]);
//       setRuns(r); setScenarios(s);
...
//     <th className="px-4 py-3 cursor-pointer hover:text-gray-200 transition-colors select-none" onClick={() => toggleSort(field)}>
//       <span className="flex items-center gap-1">
//         {label}
//         {sortKey === field && <ArrowUpDown size={12} className="text-cyan-400" />}
//       </span>
//     </th>
//   );
...
//         <select value={filterSim} onChange={(e) => setFilterSim(e.target.value)} className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-sm text-gray-300">
//           <option value="">All simulations</option>
//           {scenarios.map((s) => <option key={s.id} value={s.name}>{s.name}</option>)}
//         </select>
//         <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="bg-gray-900 border border-gray-700 rounded-lg px-3 py-1.5 text-sm text-gray-300">
...
//         </select>
//         <span className="text-sm text-gray-500 ml-auto">{runs.length} runs</span>
//       </div>

//       {loading ? (
//         <div className="text-center py-16 text-gray-500">Loading...</div>
//       ) : runs.length === 0 ? (
//         <div className="text-center py-16 text-gray-500"><p>No runs match the filters.</p></div>
//       ) : (
//         <div className="rounded-lg border border-gray-800 overflow-hidden">
//           <table className="w-full text-sm">
//             <thead>
//               <tr className="bg-gray-900/50 text-gray-400 text-xs uppercase tracking-wider text-left">
...
//               {sortedRuns.map((run) => (
//                 <tr key={run.id} onClick={() => navigate(`/run/${run.id}`)} className="hover:bg-gray-900/40 cursor-pointer transition-colors">
//                   <td className="px-4 py-3 font-medium">{run.simulation_name}</td>
//                   <td className="px-4 py-3"><StatusBadge status={run.status} /></td>
//                   <td className="px-4 py-3 text-right tabular-nums text-gray-300">{(run.duration_seconds ?? 0).toFixed(3)}s</td>
//                   <td className="px-4 py-3 text-right tabular-nums text-gray-400">{run.total_events}</td>
//                   <td className="px-4 py-3 text-right tabular-nums">
//                     <span className={run.error_count > 0 ? "text-red-400" : "text-gray-600"}>{run.error_count}</span>
//                   </td>
//                   <td className="px-4 py-3 text-gray-500">{run.started_at ? new Date(run.started_at).toLocaleString() : "—"}</td>
//                 </tr>
//               ))}
//             </tbody>
//           </table>
...
//   const SortHeader = ({ label, field }: { label: string; field: SortKey }) => (
//     <th 
//       className="px-6 py-4 cursor-pointer hover:bg-slate-200/50 transition-colors select-none font-heebo text-[11px] font-black tracking-wider text-slate-400 uppercase text-left" 
//       onClick={() => toggleSort(field)}
//     >
//       <span className="flex items-center gap-1.5">
//         {label}
//         {sortKey === field && (
//           <ArrowUpDown size={12} className="text-navy-950 animate-pulse" />
//         )}
//       </span>
//     </th>
//   );
...
//             <p className="text-xs text-slate-400 mt-1 font-medium">Review and analyze system execution logs</p>
//           </div>
//           <span className="text-[11px] font-black text-slate-400 tracking-widest uppercase bg-white border border-slate-200 px-3 py-1.5 rounded-[6px] shadow-sm">
//             {runs.length} Runs Total
//           </span>
//         </div>

//         {/* פילטרים ומסננים - תואם במדויק לפיגמה (רקע כחול כהה לסלקטורים) */}
//         <div className="flex gap-3 items-center bg-white p-4 rounded-[6px] border-2 border-slate-100 shadow-sm">
//           <Filter size={15} className="text-slate-400 mr-1" />
          
//           <select 
//             value={filterSim} 
//             onChange={(e) => setFilterSim(e.target.value)} 
//             className="bg-navy-950 border border-navy-900 rounded-[6px] px-4 py-1.5 text-xs font-bold text-white outline-none cursor-pointer hover:bg-navy-900 transition-all shadow-sm"
//           >
//             <option value="">All simulations</option>
//             {scenarios.map((s) => (
...
//         {/* טבלת הנתונים */}
//         {loading ? (
//           <div className="bg-white rounded-[6px] border-2 border-slate-100 py-24 text-center text-slate-300 font-bold text-sm tracking-widest uppercase">
//             Loading System Logs...
//           </div>
//         ) : runs.length === 0 ? (
//           <div className="bg-white rounded-[6px] border-2 border-slate-100 py-24 text-center text-slate-400 font-bold text-sm tracking-widest uppercase">
...
//             <table className="w-full text-sm">
//               <thead>
//                 {/* צבע רקע ראש טבלה אפרפר מט עדין */}
...
//                   >
//                     {/* שם הסימולציה - כהה ובולט */}
//                     <td className="px-6 py-4 font-bold text-navy-950 text-xs">
//                       {run.simulation_name}
//                     </td>
                    
//                     {/* סטטוס (משתמש ב-StatusBadge המעוצב שלך) */}
//                     <td className="px-6 py-4">
//                       <StatusBadge status={run.status} />
//                     </td>
                    
//                     {/* משך זמן הרצה */}
//                     <td className="px-6 py-4 tabular-nums text-xs text-slate-400 font-medium">
...
//                     {/* כמות שגיאות - עיצוב אחיד ונקי ללא אדום צעקני */}
//                     <td className="px-6 py-4 tabular-nums text-xs font-bold text-slate-500">
//                       {run.error_count}
//                     </td>
